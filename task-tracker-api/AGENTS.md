...
- `DELETE /tasks/{id}`: previously `storage.delete_task` existed but was unused, and `tests/test_tasks.py` expected DELETE behavior that returned 405. A `DELETE /tasks/{task_id}` route was added to `app/api/routes/tasks.py` (204 on success, 404 if the task id doesn't exist), following the same pattern as `get_task`/`update_task`. **Re-run** `pytest -v` **to confirm both previously-failing tests now pass before treating this as verified.**
