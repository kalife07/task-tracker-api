...
A learning-focused REST API (FastAPI, Python) for tracking tasks on a Kanban-style board, backed by **a JSON file** (not a database — `app/storage.py` keeps a plain `_tasks` dict in memory and mirrors it to `app/storage/storage.json` on every write, so data survives a restart). There is a standalone vanilla-JS frontend (`frontend/index.html`) that talks to the API directly, plus a set of React components under `frontend/src/components/` that are unit-tested with Jest but are **not currently wired into `index.html`** — treat them as an in-progress port, not the live UI.
...
- `test:api` uses `tests/globalSetup.js` / `globalTeardown.js`, which **spawn a real `uvicorn` process** (via `venv/Scripts/python.exe` if present, else `python`) with `APP_ENV=test` and `STORAGE_FILE=tests/.test-storage.json` (deleted before and after the run), wait for `/health`, and kill it after the run. It talks to the server over HTTP with `supertest`, not by importing the app.
...
- Python tests run directly against the FastAPI app with `TestClient` (no server process):
  ...
  ```
  `tests/conftest.py` has an autouse fixture that points storage at a per-test `tmp_path/storage.json` (via `storage._use_storage_path`) and calls `storage._reset()` before/after every test, plus `client` and `created_task` fixtures. `tests/test_storage_persistence.py` covers the JSON file itself — reload-after-restart, atomic write contents, corrupt-file quarantine.
- To run a single JS test: `npx jest -t "test name"` or `npx jest tests/tasks.test.js`.

## Architecture

Request flow: `app/api/routes/*.py` (FastAPI routers, registered in `app/main.py`) → `app/storage.py` (CRUD over the JSON-backed task dict) → `app/task_query.py` (filter predicates for `GET /tasks`) and `app/business_rules.py` (status-transition rules), with cross-cutting field validation in `app/validation.py` used from Pydantic validators in `app/models.py`.
